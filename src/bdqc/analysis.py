...
STATUS_INCOMPARABLE_STATS = 1
STATUS_AMBIGUOUS_TYPES    = 2
STATUS_NULL_OUTLIERS      = 3
STATUS_VALUE_OUTLIERS     = 4

STATUS_MSG = [
	"no anomalies detected",
	"not all statistics present for all files",
	"ambiguous data types in summary matrix",
...
class Matrix(object):
	...
	def analyze( self ):
		...
		if self.incomparables:
			...
			self.status = STATUS_INCOMPARABLE_STATS
			return self.status
		...
		if len(self.anom_col) > 0:
			...
			self.status = STATUS_AMBIGUOUS_TYPES
			return self.status
...
def analyze( args, output ):
	...
	m.analyze()
	if m.status == STATUS_VALUE_OUTLIERS:
